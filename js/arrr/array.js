/*
	arrr/array -- a stand-alone module for using Javascript 1.6 array iterators
	...
	define(['arrr/array'], function (array) {
		var items = [1, 2, 3];
	...
	older js engines by calling arrr.augmentArrayPrototype().
...
(function (global, define, undef) {
	...
	function has (feature) {
		var method = feature.substr(6);
		return isFunction(arrayPrototype[method]);
	}
	...
	if (!has('array-foreach') || !has('array-every') || !has('array-some')) {
		iterate = function (arr, lambda, context) {
			...
			for (i = 0; i < len; i++) {
				if (i in alo) {
					if (!lambda.call(context, alo[i], i, alo)) {
						return false;
					}
				}
			}
...
// TODO: finish lastIndexOf
	...
	thisModule['augmentArrayPrototype'] = function () {
